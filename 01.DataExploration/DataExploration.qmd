...
merged_all <- merged_all %>%
  mutate(
    RPTT_W1 = rowSums(across(
      c(CW1_RISK, CW1_PATIENT, CW1_TRUST, CW1_TRUSTPOLP)
    ), na.rm = TRUE),
    
    RPTT_W2 = rowSums(across(
      c(CW2_RISK, CW2_PATIENT, CW2_TRUST, CW2_TRUSTPOLP)
    ), na.rm = TRUE),
    
    RPTT_W3 = rowSums(across(
      c(CW3_RISK, CW3_PATIENT, CW3_TRUST, CW3_TRUSTPOLP)
    ), na.rm = TRUE)
  )
...
merged_all <- merged_all %>%
  mutate(Lonely_W1 = rowSums(across(
...
merged_all <- merged_all %>%
  mutate(
    WEMWBS_W1 = rowSums(across(all_of(paste0("CW1_WEMWBS_", 1:7)), recode_wemwbs), na.rm = TRUE),
    WEMWBS_W2 = rowSums(across(all_of(paste0("CW2_WEMWBS_", 1:7)), recode_wemwbs), na.rm = TRUE),
    WEMWBS_W3 = rowSums(across(all_of(paste0("CW3_WEMWBS_", 1:7)), recode_wemwbs), na.rm = TRUE)
  )
...
merged_all <- merged_all %>%
  mutate(
    GHQ_W1 = rowSums(across(all_of(paste0("CW1_GHQ12", 1:12)), recode_GHQ), na.rm = TRUE),
    GHQ_W2 = rowSums(across(all_of(paste0("CW2_GHQ12", 1:12)), recode_GHQ), na.rm = TRUE),
    GHQ_W3 = rowSums(across(all_of(paste0("CW3_GHQ12", 1:12)), recode_GHQ), na.rm = TRUE)
  )
```
...
```{r, include=FALSE}
sapply(merged_all, function(x) mean(is.na(x)))
```
...
fit_long <- sem(long_model, data = merged_all, estimator = "MLR", missing = "fiml")
