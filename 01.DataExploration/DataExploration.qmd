...
W01_complete <- W01_complete[!duplicated(W01_complete$NCDSID), ]
W02_complete <- W02_complete[!duplicated(W02_complete$NCDSID), ]

W01_clean <- W01_complete %>% group_by(NCDSID) %>% slice(1) %>% ungroup()
W02_clean <- W02_complete %>% group_by(NCDSID) %>% slice(1) %>% ungroup()
W03_clean <- W03_complete %>% group_by(NCDSID) %>% slice(1) %>% ungroup()

merged_all <- W01_clean %>%
  inner_join(W02_clean, by = "NCDSID") %>%
  inner_join(W03_clean, by = "NCDSID")


n_distinct(merged_all$NCDSID)
nrow(merged_all) 


```

Save the final data set for next analysis

```{r}
save(merged_all, file = "Data.RData")
