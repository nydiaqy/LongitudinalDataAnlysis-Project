...
=======
|-----------------|-----------------|---------------------------------------|
>>>>>>> eed9531ff27ea24ed67a3816a0cdf3c08361611b
...
# Step 1: Select relevant variables and ensure NCDSID is included
W01_A <- W01 %>%
  select(NCDSID, starts_with("CW1_"))
W02_A <- W02 %>%
  select(NCDSID, starts_with("CW2_"))
W03_A <- W03 %>%
  select(NCDSID, starts_with("CW3_"))

# Step 2: Identify common IDs across all three waves
common_ids <- Reduce(intersect, list(W01_A$NCDSID, W02_A$NCDSID, W03_A$NCDSID))

# Step 3: Filter to only keep rows with common IDs
W01_complete <- W01_A %>%
...
# Step 4: Check for duplicated IDs (should ideally all be unique)
table(table(W01_complete$NCDSID))
table(table(W02_complete$NCDSID))
table(table(W03_complete$NCDSID))

# Optionally view which IDs are duplicated (should return 0 rows ideally)
W01_complete %>% filter(duplicated(NCDSID) | duplicated(NCDSID, fromLast = TRUE)) %>% arrange(NCDSID)
W02_complete %>% filter(duplicated(NCDSID) | duplicated(NCDSID, fromLast = TRUE)) %>% arrange(NCDSID)
W03_complete %>% filter(duplicated(NCDSID) | duplicated(NCDSID, fromLast = TRUE)) %>% arrange(NCDSID)

# Step 5: Remove duplicated IDs
W01_complete <- W01_complete %>% distinct(NCDSID, .keep_all = TRUE)
W02_complete <- W02_complete %>% distinct(NCDSID, .keep_all = TRUE)
W03_complete <- W03_complete %>% distinct(NCDSID, .keep_all = TRUE)

# Step 6: Merge all three waves by NCDSID
merged_all <- W01_complete %>%
  inner_join(W02_complete, by = "NCDSID", suffix = c("_w1", "_w2")) %>%
  inner_join(W03_complete, by = "NCDSID")  # You can add suffix for wave3 if needed
...
```
```{r}
## Variable Overview Table (for SEM preparation)

library(tibble)
library(knitr)

var_table <- tribble(
  ~Variable_Name,       ~Wave,     ~Original_Var_Name, ~Type,         ~Description,                      ~Reverse_Coded, ~Missing_N,
  "gender",             "Wave 1",  "SEXW1",             "categorical", "1 = Male, 2 = Female",            "No",           5,
  "longterm_illness",   "Wave 1",  "NCD",               "binary",      "1 = Yes, 0 = No",                 "No",           0,
  "depression_score",   "Wave 2",  "DEP_W2",            "continuous",  "PHQ-9 total score",               "No",           2,
  "anxiety_score",      "Wave 3",  "ANX_W3",            "continuous",  "GAD-7 total score",               "No",           3
)

kable(var_table, caption = "Summary of Variables for SEM")
```
